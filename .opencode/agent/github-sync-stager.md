...
- Excluir de GitHub todas las imagenes ubicadas en `test_docs`.
...
powershell -NoProfile -Command "$ext='.png','.jpg','.jpeg','.gif','.bmp','.webp','.tif','.tiff','.avif','.heic','.heif','.ico','.svg'; Get-ChildItem -LiteralPath 'E:\DesarrolloAI\github_update_local_ai\repo\test_docs' -File -Recurse | Where-Object { $ext -contains $_.Extension.ToLowerInvariant() } | Remove-Item -Force"
git rm -r --ignore-unmatch test_docs/*.png test_docs/*.jpg test_docs/*.jpeg test_docs/*.gif test_docs/*.bmp test_docs/*.webp test_docs/*.tif test_docs/*.tiff test_docs/*.avif test_docs/*.heic test_docs/*.heif test_docs/*.ico test_docs/*.svg test_docs/**/*.png test_docs/**/*.jpg test_docs/**/*.jpeg test_docs/**/*.gif test_docs/**/*.bmp test_docs/**/*.webp test_docs/**/*.tif test_docs/**/*.tiff test_docs/**/*.avif test_docs/**/*.heic test_docs/**/*.heif test_docs/**/*.ico test_docs/**/*.svg
git status
git add .
...
- Despues de `robocopy`, elimina del staging Git las imagenes de `test_docs` aunque ya estuvieran sincronizadas en GitHub.
...
- Nunca subas imagenes dentro de `test_docs`; elimina del staging cualquier `*.png`, `*.jpg`, `*.jpeg`, `*.gif`, `*.bmp`, `*.webp`, `*.tif`, `*.tiff`, `*.avif`, `*.heic`, `*.heif`, `*.ico` o `*.svg` en esa carpeta.
- No uses comandos destructivos como `git reset --hard`, `git clean`, `del`, `Remove-Item` o variantes de borrado salvo para eliminar imagenes dentro de `E:\DesarrolloAI\github_update_local_ai\repo\test_docs` o con autorizacion explicita del usuario.
